##  DoD Attack Drone Convoy Tracking System (Rust) — Dioxus Edition

The provided Rust application is a full-stack DoD attack drone convoy tracking system using a WebAssembly native Rust **Dioxus 0.7** frontend reading realtime drone status telemetry, GraphQL drone leader tracking on the server-side using Redis RW cache cluster and ScyllaDB NoSQL DB (ScyllaDB is 100% rewrite of Cassandra DB NoSQL in C++).

This is the **companion edition** to the Leptos build (`drone-convoy-attack-tracker-leptos-rs`). The two share the ENTIRE backend byte-for-byte — domain, persistence, GraphQL API, simulator/convoy service, schema, deploy — and the frontends are pixel-identical: same `main.css`, same assets, same DOM. Only the UI framework differs. That is the point of the pair: the same system, twice, so the framework comparison is real.
...
```shell
drone-convoy-attack-tracker-dioxus-rs
├── Cargo.lock
├── Cargo.toml
├── Makefile
├── README.md
...
│   ├── drone-frontend             # Dioxus 0.7 (web) — the only crate that differs from the Leptos edition
│   │   ├── Cargo.toml
│   │   ├── Dioxus.toml            # dx project config (replaces Trunk.toml)
│   │   ├── assets/                # crate-local favicon copy served by dx (synced from ../../assets/images)
│   │   ├── index.html             # #main mount root; Leaflet + ECharts script tags
│   │   ├── src
│   │   │   ├── components
│   │   │   │   ├── charts.rs
│   │   │   │   ├── drone_card.rs
...
│   │   │   ├── main.rs
│   │   │   ├── services
│   │   │   │   ├── api.rs         # UNCHANGED from Leptos: plain gloo-net + serde
│   │   │   │   └── mod.rs
│   │   │   └── state
│   │   │       └── mod.rs
│   │   └── style
...
drone-simulator ──GraphQL mutations──▶ drone-graphql-api ──▶ drone-persistence ──▶ ScyllaDB
  (convoy service)   telemetry/state/        │                      │
        ...
        │                                    ▼
        │  reads tasking order      drone-frontend (Dioxus/WASM)
...
## Leptos → Dioxus: What Changed and What Didn't

**Didn't change (byte-identical to the Leptos edition):** `drone-domain`,
`drone-persistence`, `drone-graphql-api`, `drone-analytics`, `drone-simulator`,
`schema/cql/*`, `config/`, `containers/nginx.conf`, the Helm chart templates,
`assets/images/*`, `crates/drone-frontend/style/main.css`, and
`crates/drone-frontend/src/services/api.rs`. If a pixel differs between the
two builds at the same viewport, that is a bug.

**Changed:** the nine files under `crates/drone-frontend/src/` that render
(components, `state`, `lib.rs`), the crate manifest, `Dioxus.toml` in place of
`Trunk.toml`, `index.html`'s mount root, and the toolchain (`dx` in place of
`trunk` in the Makefile and `Containerfile.frontend`). Every `make` command is
the same.

**The translation, construct for construct:**

| Leptos 0.7                                | Dioxus 0.7                                                    |
|---|---|
| `#[component] fn X() -> impl IntoView`    | `#[component] fn X() -> Element`                              |
| `view! { <div class="a">..</div> }`       | `rsx! { div { class: "a", .. } }`                             |
| `RwSignal<T>` fields in `AppState`        | `Signal<T>` fields; `AppState: Copy`; provided via `use_context_provider` |
| `sig.get()` / `sig.set()` / `.update()`   | `sig()` or `sig.read()` / `sig.set()` / `sig.write()`         |
| `Effect::new(move \|_\| ..)`             | `use_effect(move \|\| ..)` — auto-tracks reads               |
| `<For each key children>`                 | `for item in items { Row { key: "{k}", .. } }`               |
| `class:selected=is_selected`              | `class: if is_selected { "card selected" } else { "card" }`   |
| `inner_html=svg`                          | `dangerous_inner_html: "{svg}"`                               |
| `spawn_local(async ..)`                   | `spawn(async ..)` (Dioxus runtime)                            |
| `set_interval` via web-sys `Closure`      | `spawn` + `gloo_timers::future::sleep` loop                   |
| `StoredValue<T>`                          | `use_signal` / `use_hook(\|\| Rc<RefCell<..>>)`               |
| Trunk `data-trunk` copy-file / css        | `asset!("/style/main.css")` + `document::Stylesheet`          |

**Three Dioxus rules learned by running it** (each cost a real bug):
1. **Never write a signal during render.** It re-triggers the render that owns
   the hooks and restarts their futures. Initial state goes inside a
   once-only `use_future`.
2. **Never hold a `.read()`/`.peek()` guard across an `.await`**, or across a
   `set()` on another signal. Copy the value out, drop the guard, then await.
3. **Never write a signal from a raw JS callback** (`setInterval`, `setTimeout`,
   a Leaflet event). Use `spawn` + sleep, `use_future`, or Dioxus event
   handlers; a raw callback firing mid-render trips the runtime lock
   ("RefCell already borrowed"). Leptos's lock-free signals tolerated this;
   Dioxus's scheduler-owned ones do not.

**Dev experience difference you will feel:** `dx serve` hot-patches `rsx!`
and CSS edits into the running app in milliseconds without a reload (state
survives); Rust logic changes rebuild incrementally and reload. Trunk always
did a full rebuild + reload.
...
| Dioxus CLI (`dx`) | 0.7.x | WASM build + dev server w/ hot reload (`cargo install dioxus-cli --locked`; `make setup` does it) |
...
No Node, no npm, no Python: the frontend toolchain is entirely Rust. Do not
build on Dioxus 0.8 (alpha at time of writing); this edition targets 0.7.
...
make serve          # starts ScyllaDB+Redis (podman), applies schema, then runs — natively,
                    #   in parallel — the API (release), the dx dev server (hot reload), and
                    #   the CONVOY SERVICE (the simulator, flying sorties back to back)
```
...
- Rust Leptos (the companion edition)
https://leptos.dev/
